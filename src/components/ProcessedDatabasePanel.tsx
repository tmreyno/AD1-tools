...
const ProcessedDatabasePanel: Component<ProcessedDatabasePanelProps> = (props) => {
  ...
  return (
    <div class="flex flex-col h-full bg-bg-panel rounded overflow-hidden">
      {/* Header with actions */}
      <div class="flex items-center justify-between px-3 py-2 bg-bg-card border-b border-border shrink-0">
        <h3 class="m-0 text-sm font-semibold text-txt">📊 Processed Databases</h3>
        <div class="flex gap-1">
          <button 
            class="bg-transparent border border-transparent rounded px-2 py-1 cursor-pointer text-sm transition-all duration-150 hover:bg-bg-hover hover:border-border disabled:opacity-50 disabled:cursor-not-allowed" 
          ...
          <Show when={databases().length > 0}>
            <button 
              class="bg-transparent border border-transparent rounded px-2 py-1 cursor-pointer text-sm transition-all duration-150 hover:bg-bg-hover hover:border-border" 
              onClick={clearAll}
              title="Clear all"
            >
              🗑️
      ...
      {/* Loading indicator */}
      <Show when={loading()}>
        <div class="flex items-center justify-center gap-2 p-4 text-txt-muted text-base">
          <span class="animate-spin-slow">⏳</span> Scanning...
        </div>
      </Show>

      {/* Error message */}
      <Show when={error()}>
        <div class="flex items-center justify-between px-3 py-2 m-2 bg-error-soft border border-error/30 rounded text-error text-sm">
          <span>⚠️ {error()}</span>
          <button class="bg-transparent border-none px-1.5 py-0.5 cursor-pointer text-xs opacity-70 hover:opacity-100 transition-opacity" onClick={() => setError(null)}>✕</button>
        </div>
      </Show>

      {/* Empty state */}
      <Show when={databases().length === 0 && !loading()}>
        <div class="flex flex-col items-center justify-center px-4 py-8 text-center text-txt-muted flex-1">
          <p class="my-1 text-base">No processed databases loaded</p>
          <p class="text-xs text-txt-faint">Click 🔍 to scan a folder or ➕ to add files</p>
          <p class="text-xs text-txt-faint">Supports: AXIOM, Cellebrite PA, X-Ways, Autopsy, EnCase, FTK</p>
        </div>
      </Show>
          ...
          {(db) => {
            ...
            return (
              <div class={`border-b border-border last:border-b-0 ${isSelected() ? 'bg-accent-soft' : ''}`}>
                ...
                <div 
                  class="flex items-start gap-2 py-2.5 px-2 cursor-pointer transition-colors duration-150 hover:bg-bg-hover"
                  onClick={() => {
                    selectDatabase(db);
                    setExpanded(!expanded());
                  }}
                >
                  <span class={`text-2xs text-txt-faint transition-transform duration-150 shrink-0 w-3.5 text-center mt-1 ${expanded() ? 'rotate-90' : ''}`}>
                    ▶
                  </span>
                  <span class="text-xl shrink-0 leading-none">{getDbTypeIcon(db.db_type)}</span>
                  <div class="flex-1 min-w-0 flex flex-col gap-0.5">
                    <div class="text-base font-semibold text-txt whitespace-nowrap overflow-hidden text-ellipsis">{displayName()}</div>
                    <div class="flex flex-wrap gap-2 text-xs text-txt-muted">
                    ...
                    <Show when={caseInfo()?.examiner || db.examiner}>
                      <div class="text-xs text-txt-faint">👤 {caseInfo()?.examiner || db.examiner}</div>
                    </Show>
                  </div>
                  <div class={`flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100 ${isLoading() ? 'opacity-100' : 'hover:opacity-100'}`}>
                    ...
                    <button 
                      class="bg-transparent border-none px-1.5 py-0.5 cursor-pointer text-xs text-txt-faint opacity-70 hover:opacity-100 hover:text-error transition-all"
                      onClick={(e) => {
                        e.stopPropagation();
                ...
                <Show when={expanded() && caseInfo()}>
                  <div class="pl-4 pb-1.5 border-l border-border ml-[22px]">
                    {/* Case Report */}
                    <div 
                      class={`flex items-center gap-2 px-2.5 py-1.5 cursor-pointer rounded text-base text-txt transition-all duration-150 hover:bg-bg-hover ${currentDetailView()?.type === 'case' ? 'bg-accent-soft text-accent' : ''}`}
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    {/* Evidence Sources */}
                    <div 
                      class={`flex items-center gap-2 px-2.5 py-1.5 cursor-pointer rounded text-base text-txt transition-all duration-150 hover:bg-bg-hover ${currentDetailView()?.type === 'evidence' ? 'bg-accent-soft text-accent' : ''}`}
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    >
                      ...
                      <Show when={caseInfo()?.evidence_sources?.length}>
                        <span class="text-xs text-txt-faint">({caseInfo()!.evidence_sources.length})</span>
                      </Show>
                    </div>
                    ...
                    <div>
                      <div 
                        class={`flex items-center gap-2 pl-1 pr-2.5 py-1.5 cursor-pointer rounded text-base text-txt transition-all duration-150 hover:bg-bg-hover ${currentDetailView()?.type === 'keywords' ? 'bg-accent-soft text-accent' : ''}`}
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      >
                        <span class={`text-2xs text-txt-faint transition-transform duration-150 w-3 text-center ${keywordsExpanded() ? 'rotate-90' : ''}`}>
                          ▶
                        </span>
                        <span class="text-sm shrink-0">🔑</span>
                        <span class="flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap">Keywords</span>
                        <span class="text-xs text-txt-faint">({totalKeywords()})</span>
                      </div>
                        ...
                        <div class="pl-5 mt-0.5">
                          <For each={keywordFiles()}>
                            {(file: AxiomKeywordFile) => {
                              const isActive = () => currentDetailView()?.type === 'keyword-file' && 
                                          (currentDetailView() as any)?.file?.file_name === file.file_name;
                              return (
                                <div 
                                  class={`flex items-center gap-1.5 px-2 py-1 cursor-pointer rounded-sm text-sm text-txt-muted transition-all duration-150 hover:bg-bg-hover hover:text-txt ${isActive() ? 'bg-accent-soft text-accent' : ''}`}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                ...
                                >
                                  ...
                                  <span class="text-2xs text-txt-faint font-mono">{file.record_count}</span>
                                </div>
                              );
                            }}
                          </For>
                    ...
                    {/* Artifacts */}
                    <div 
                      class={`flex items-center gap-2 px-2.5 py-1.5 cursor-pointer rounded text-base text-txt transition-all duration-150 hover:bg-bg-hover ${currentDetailView()?.type === 'artifacts' ? 'bg-accent-soft text-accent' : ''}`}
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    >
                      ...
                      <Show when={caseInfo()?.total_artifacts}>
                        <span class="text-xs text-txt-faint">({caseInfo()!.total_artifacts.toLocaleString()})</span>
                      </Show>
                    </div>
                  </div>
                </Show>
      ...
      {/* Summary footer */}
      <Show when={databases().length > 0}>
        <div class="px-3 py-1.5 bg-bg-card border-t border-border text-xs text-txt-faint shrink-0">
