...
fn open_axiom_db(path: &Path) -> Result<Connection, String> {
    Connection::open_with_flags(path, OpenFlags::SQLITE_OPEN_READ_ONLY)
        .map_err(|e| format!("Failed to open AXIOM database: {}", e))
}
...
pub fn parse_axiom_case(path: &Path) -> Result<AxiomCaseInfo, String> {
...
/// </Case>
/// ```
fn parse_mcfc_file(path: &Path) -> Result<AxiomCaseInfo, String> {
    let file = fs::File::open(path)
        .map_err(|e| format!("Failed to open .mcfc file: {}", e))?;
...
/// </CaseSummary>
/// ```
fn parse_case_information_xml(path: &Path) -> Result<AxiomCaseInfo, String> {
    let file = fs::File::open(path)
        .map_err(|e| format!("Failed to open Case Information.xml: {}", e))?;
    let reader = BufReader::new(file);
    let mut xml_reader = Reader::from_reader(reader);
    xml_reader.config_mut().trim_text(true);
...
fn find_main_mfdb(dir: &Path) -> Result<std::path::PathBuf, String> {
    ...
    Err("No .mfdb file found in AXIOM case folder".to_string())
}

/// Query case information from AXIOM database
fn query_case_info(conn: &Connection) -> Result<(String, Option<String>, Option<String>, Option<String>, Option<String>), String> {
...
fn query_evidence_sources(conn: &Connection) -> Result<Vec<AxiomEvidenceSource>, String> {
...
fn query_keyword_info(conn: &Connection) -> Result<AxiomKeywordInfo, String> {
                ...
                for kw in keywords {
                    let keyword = AxiomKeyword {
                        ...
                        encoding_types: kw.get("EncodingTypes")
                            ...
                            .map(|arr| arr.iter()
                                .filter_map(|e| e.as_i64().map(|n| encoding_type_to_string(n)))
                                .collect())
                            .unwrap_or_default(),
...
fn count_total_artifacts(conn: &Connection) -> Result<u64, String> {
...
pub fn get_artifact_categories(path: &Path) -> Result<Vec<ArtifactCategorySummary>, String> {
    let mfdb_path = if path.is_dir() {
        find_main_mfdb(path)?
    } else {
        path.to_path_buf()
