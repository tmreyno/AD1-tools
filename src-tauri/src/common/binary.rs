...
pub fn read_u8(file: &mut File) -> Result<u8, String> {
    let mut buf = [0u8; 1];
...
pub fn read_u16_le(file: &mut File) -> Result<u16, String> {
    let mut buf = [0u8; 2];
    file.read_exact(&mut buf)
...
pub fn read_u32_le(file: &mut File) -> Result<u32, String> {
    let mut buf = [0u8; 4];
    file.read_exact(&mut buf)
...
pub fn read_u64_le(file: &mut File) -> Result<u64, String> {
    let mut buf = [0u8; 8];
    file.read_exact(&mut buf)
...
#[allow(dead_code)]
pub fn read_u32_be(file: &mut File) -> Result<u32, String> {
    let mut buf = [0u8; 4];
    file.read_exact(&mut buf)
        .map_err(|e| format!("Failed to read u32 BE: {}", e))?;
    Ok(u32::from_be_bytes(buf))
}

/// Read u64 big-endian from file at current position
#[allow(dead_code)]
pub fn read_u64_be(file: &mut File) -> Result<u64, String> {
    let mut buf = [0u8; 8];
    file.read_exact(&mut buf)
        .map_err(|e| format!("Failed to read u64 BE: {}", e))?;
...
pub fn read_string_at(file: &mut File, offset: u64, length: usize) -> Result<String, String> {
    if length == 0 {
...
pub fn read_string(file: &mut File, length: usize) -> Result<String, String> {
    if length == 0 {
        return Ok(String::new());
    }
...
pub fn read_string_with_u32_length(file: &mut File) -> Result<String, String> {
...
pub fn read_bytes_at(file: &mut File, offset: u64, length: usize) -> Result<Vec<u8>, String> {
    if length == 0 {
        return Ok(Vec::new());
    }
    file.seek(SeekFrom::Start(offset))
...
pub fn read_bytes(file: &mut File, length: usize) -> Result<Vec<u8>, String> {
    if length == 0 {
        return Ok(Vec::new());
    }
    let mut buf = vec![0u8; length];
    file.read_exact(&mut buf)
        .map_err(|e| format!("Failed to read {} bytes: {}", length, e))?;
    Ok(buf)
}
