...
fn main() -> Result<(), String> {
    ...
    println!("Media size:       {} bytes ({:.2} GB)", 
             media_size, media_size as f64 / 1_073_741_824.0);
    ...
    if mbr.len() >= 512 && mbr[510] == 0x55 && mbr[511] == 0xAA {
        ...
        for i in 0..4 {
            ...
            let start_lba = u32::from_le_bytes([
                mbr[offset + 8], mbr[offset + 9], 
                mbr[offset + 10], mbr[offset + 11]
            ]);
            let size_sectors = u32::from_le_bytes([
                mbr[offset + 12], mbr[offset + 13], 
                mbr[offset + 14], mbr[offset + 15]
            ]);
            ...
            if partition_type != 0 {
                ...
                println!("Partition {}: Type=0x{:02X} ({}) Boot={} Start=LBA {} Size={:.2} GB",
                         i + 1, partition_type, type_name,
                ...
                if partition_type == 0x07 || partition_type == 0x0B || partition_type == 0x0C {
                    ...
                    println!("  → Reading partition boot sector at offset {}...", partition_offset);
                    ...
                    if let Ok(boot_sector) = handle.read_at(partition_offset, 512) {
                        ...
                        if boot_sector.len() >= 8 && boot_sector[3..7] == *b"NTFS" {
                            ...
                            let mft_offset = partition_offset + (mft_cluster * sectors_per_cluster as u64 * bytes_per_sector as u64);
                            println!("  → $MFT at cluster {} (offset {})", mft_cluster, mft_offset);
